...
This fork adds the concept of intra-job dependencies, or "job prerequisites."

If you need to support certain jobs starting only after other jobs have completed, you're in the right place.

Please note that this has only been tested with ActiveRecord and MySQL.
...
h2. Prerequisite Jobs

Note that @.delay.method(params)@ returns a numeric job identifier.

By providing an array of prerequisite job IDs to a delay call, you prevent that method from being invoked until the specified jobs have completed:

<pre>
prereqs = []
prereqs << self.delay.method_a
self.delay(:prerequisites=>prereqs).method_b
</pre>
